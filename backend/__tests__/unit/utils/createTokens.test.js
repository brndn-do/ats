...
describe('Error handling', () => {
  ...
  it('should throw an error if userId is missing or not a number', () => {
    expect(() => createTokens(undefined, username, isAdmin)).toThrow('Invalid arguments');
    expect(() => createTokens(null, username, isAdmin)).toThrow('Invalid arguments');
    expect(() => createTokens('1', username, isAdmin)).toThrow('Invalid arguments');
  });

  it('should throw an error if username is missing or not a string', () => {
    expect(() => createTokens(userId, undefined, isAdmin)).toThrow('Invalid arguments');
    expect(() => createTokens(userId, null, isAdmin)).toThrow('Invalid arguments');
    expect(() => createTokens(userId, 123, isAdmin)).toThrow('Invalid arguments');
  });

  it('should throw an error if isAdmin is missing or not a boolean', () => {
    expect(() => createTokens(userId, username, undefined)).toThrow('Invalid arguments');
    expect(() => createTokens(userId, username, null)).toThrow('Invalid arguments');
    expect(() => createTokens(userId, username, 'false')).toThrow('Invalid arguments');
  });
});
