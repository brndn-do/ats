...
The backend’s dependencies are installed inside the backend Docker container. You only need to install them locally if:
...
Then, use `docker-compose exec` to run tests inside the `backend` container. Using the `-t` flag provides a colorized output.
...
```bash
# Run only __tests__/jobs.test.js
